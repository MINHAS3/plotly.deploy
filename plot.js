function init() {
  ...
  buildGaugeCharts(sampleNames[0]);
...
function buildBarChart(sample) {
  d3.json('samples.json').then(data => {
    ...
    var y = record.otu_ids.slice(0, 10).map(otu=>`OTU ${otu}`).reverse()
    var x = record.sample_values.slice(0, 10).reverse()
    var otu_labels = record.otu_labels.slice(0, 10).reverse()
